...
bool RF24Network::appendFragmentToFrame(RF24NetworkFrame frame) {
  ...
  if ( frame.header.type == NETWORK_MORE_FRAGMENTS || frame.header.type == NETWORK_MORE_FRAGMENTS_NACK ){
	...
	if( f->header.reserved - 1 == frame.header.reserved && f->header.id == frame.header.id){	
      // Cache the fragment
      memcpy_P(f->message_buffer+f->message_size, frame.message_buffer, frame.message_size);
...
uint8_t RF24Network::enqueue(RF24NetworkHeader* header)
{
  ...
  if(isFragment){
	...
	if(header->type == NETWORK_FIRST_FRAGMENT){
        ...
        memcpy_P(&frag_queue,&frame_buffer,8);
        memcpy_P(frag_queue.message_buffer,frame_buffer+sizeof(RF24NetworkHeader),message_size);
...
IF_SERIAL_DEBUG_FRAGMENTATION_L2(  for(int i=0; i<frag_queue.message_size;i++){  Serial.println(frag_queue.message_buffer[i],HEX);  } );
	...
	if(header->type == NETWORK_LAST_FRAGMENT || header->type == NETWORK_MORE_FRAGMENTS || header->type == NETWORK_MORE_FRAGMENTS_NACK){
        ...
        memcpy_P(frag_queue.message_buffer+frag_queue.message_size,frame_buffer+sizeof(RF24NetworkHeader),message_size);
	    frag_queue.message_size += message_size;
...
IF_SERIAL_DEBUG_FRAGMENTATION_L2(for(int i=0; i< frag_queue.message_size;i++){ Serial.println(frag_queue.message_buffer[i],HEX); }  );		
        ...
        if(MAX_PAYLOAD_SIZE - (next_frame-frame_queue) >= frag_queue.message_size){
          memcpy_P(next_frame,&frag_queue,10);
          memcpy_P(next_frame+10,frag_queue.message_buffer,frag_queue.message_size);
  ...
  if(message_size + (next_frame-frame_queue) <= MAIN_BUFFER_SIZE){
    memcpy_P(next_frame,&frame_buffer,8);
    memcpy_P(next_frame+8,&message_size,2);
    memcpy_P(next_frame+10,frame_buffer+8,message_size);
...
uint16_t RF24Network::peek(RF24NetworkHeader& header)
{
  if ( available() )
  {
    ...
    memcpy_P(&header,&frame.header,sizeof(RF24NetworkHeader));
    return frame.message_size;
  #else
	RF24NetworkFrame *frame = (RF24NetworkFrame*)(frame_queue);
    memcpy_P(&header,&frame->header,sizeof(RF24NetworkHeader));
    uint16_t msg_size;
    memcpy_P(&msg_size,frame_queue+8,2);
    return msg_size;
  #endif
  }
  return 0;
...
uint16_t RF24Network::read(RF24NetworkHeader& header,void* message, uint16_t maxlen)
{
  uint16_t bufsize = 0;
   ...
   if ( available() ) {
    ...
    memcpy_P(&header,&(frame.header),sizeof(RF24NetworkHeader));
    memcpy_P(message,frame.message_buffer,bufsize);
  ...
  if ( available() )
  {
    ...
    memcpy_P(&header,frame_queue,8);
    memcpy_P(&bufsize,frame_queue+8,2);

    if (maxlen > 0)
    {		
		maxlen = rf24_min(maxlen,bufsize);
        memcpy_P(message,frame_queue+10,maxlen);
    ...
    #endif
    memcpy_P(frame_queue,frame_queue+bufsize+10+padding,sizeof(frame_queue)- bufsize);
...
bool RF24Network::_write(RF24NetworkHeader& header,const void* message, uint16_t len, uint16_t writeDirect)
{
  ...
  memcpy_P(frame_buffer,&header,sizeof(RF24NetworkHeader));
  ...
  if (len){
    #if defined (RF24_LINUX)
    memcpy_P(frame_buffer + sizeof(RF24NetworkHeader),message,rf24_min(frame_size-sizeof(RF24NetworkHeader),len));
    ...
    memcpy_P(frame_buffer + sizeof(RF24NetworkHeader),message,len);
