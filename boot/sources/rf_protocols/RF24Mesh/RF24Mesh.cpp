...
//#if defined (ARDUINO_SAM_DUE) || defined (__linux)
	...
	int16_t address = 0;
	memcpy_P(&address,network.frame_buffer+sizeof(RF24NetworkHeader),sizeof(address));
...
int16_t RF24Mesh::getNodeID(uint16_t address){
    ...
    if(!mesh_address){ //Master Node
      ...
      if(network.write(header,&address,sizeof(address)) ){
        ...
        int16_t ID;
        memcpy_P(&ID,&network.frame_buffer[sizeof(RF24NetworkHeader)],sizeof(ID));
        return ID;
      }
    }
    return -1;
...
bool RF24Mesh::requestAddress(uint8_t level){    
    ...
    while(1){
		...
		if(network.update() == NETWORK_POLL){
            memcpy_P(&contactNode[pollCount],&network.frame_buffer[0],sizeof(uint16_t));
            ++pollCount;
  ...
  if(type != NETWORK_ADDR_RESPONSE){
	...
	//memcpy_P(&addrResponse,network.frame_buffer+sizeof(RF24NetworkHeader),sizeof(addrResponse));
    memcpy_P(&newAddress,network.frame_buffer+sizeof(RF24NetworkHeader),sizeof(newAddress));
...
void RF24Mesh::DHCP(){
  ...
  if(doDHCP){
    ...
    memcpy_P(&header,network.frame_buffer,sizeof(RF24NetworkHeader));	
